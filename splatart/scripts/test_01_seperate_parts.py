...
def process_models(input_model_dirs:list[str],\
    ...
    for manager in splat_managers:
        ...
        for partidx in [0,1,2,3]:
            ...
            print(f"example cam pose size: {example_cam_pose.shape}")
            intrinsic = torch.Tensor([[fx, 0, w/2], [0, fy, h/2], [0, 0, 1]])
            print(f"intrinsic size: {intrinsic.shape}")
            rgb, alpha, info = manager.render_parts_at_campose(example_cam_pose, intrinsic, w, h, [partidx])
            rgb = rgb.cpu().detach().numpy()[0]
